...
- **Gate closed 2026-08-02 (BP): all three criteria met.** Evidence: `spectrace init` scaffolds `.spectrace/` in 6 files and a second run writes nothing (both exit 0); `spectrace validate` reports 55 requirements, 0 violations, 0 warnings, exit 0 over `specs/requirements/`; the frozen HOOK corpus re-validates 12/12 clean with all twelve SHA-256 hashes intact; BP ran `pnpm --filter @spectrace/studio dev`, opened the vault, and confirmed the file tree and markdown previews render. Delivered: REQ-CORE-001…004 and REQ-CLI-001/002 `implemented` with acceptance-criterion-mapped tests (188 across the monorepo); `spectrace-core-spec.md` (SPEC-CORE-000) split out of the CLI spec per its §14; all 55 requirements extracted one-per-file into `specs/requirements/` with the narrative specs' tables generated from their frontmatter and guarded in CI. The `partial` status value, adopted on trial at the start of the phase, is retained — it recorded genuine mid-flight progress on REQ-CORE-011, REQ-CLI-003, and REQ-APP-001, and its definition is what kept REQ-APP-002 at `proposed` rather than overclaiming a read-only preview as live preview.
- **Carried forward, not gate-blocking:** Studio remains a walking skeleton — REQ-APP-001 AC2/AC3 (file operations, external-change detection) and all of REQ-APP-002 wait on the CodeMirror editor in setup-plan step 4.2. The `spectrace.validation-report` and `spectrace.init-report` JSON envelopes live in the CLI and should move to core when Phase D records the parity snapshots (NFR-APP-007). SPEC-APP-000 Appendix A still points at proposal steps rather than REQ IDs.
...
- **Gate closed 2026-08-04 (BP): both criteria met.** Evidence: A/B/C Recall@k reported on the frozen corpus with per-stratum rows and SVG figures, verified by eye by BP; BP ran Studio and confirmed it edits a spec file and flags a duplicate ID live. Four open items resolved the same day, three of them BP decisions and one delegated:
  - **Default retrieval mode stays Configuration A (BP).** At the k=10 operating point the gap to B is one requirement in twelve, against breaking locality, requiring an API key, and putting the corpus through a third party. B is reported as evidence that the retrieval-first bet clears its bar — a separate claim from what ships.
  - **REQ-CLI-009 AC2 amended as recommended (BP):** scoped to `evaluate retrieval` and to metric computation, with `compare` and `sweep` named in the statement.
  - **REQ-CORE-023 scope — "a model" means any model, embedding models included (BP delegated to Claude).** Configuration B was therefore in violation as written, not merely under-reported: the statement now names B and C as an explicit exception that is never the default, is disclosed before it runs, and is refused without the operator's explicit acceptance (new AC3). The §1 locality claim and NFR-CORE-005 are qualified to match. Keeping A as the default is what holds the exception narrow.
  - **Transmitted-content log under-reporting fixed (BP instruction):** the log now records retrieval-time embedding — model, corpus-wide text counts, and the network/cache split (new AC2, artifact version 2).
- **Carried forward, not gate-blocking:** `--dry-run` still permits retrieval to reach the network in semantic/hybrid mode (now gated by AC3, so never silently); making it imply cache-only is a runtime-contract change left to BP. Consent is per-run via `--accept-corpus-transmission`; a repo-level config key was considered and deliberately not added, since a flag keeps consent visible in every recorded command line. REQ-CORE-011 AC2 and REQ-CLI-004's cost estimate still wait on Phase D, and the REQ-APP statuses stay deliberately conservative.
...
- **Descoped (2026-08-11, BP): REQ-APP-010/011 (GitHub connect + SHA-keyed sync cache) move to R1.1 (post-capstone), joining the §9 R1.1 scope.** REQ-APP-015 (link a local repository directory, shipped this phase) covers the capstone's actual need — a vault analyzing a codebase — for every remaining phase, including the drift scenarios (Phase F) and the full dogfood (Phase G), both of which run on local checkouts anyway. The decisions REQ-APP-010/011 inherit were made and recorded with REQ-APP-015: artifacts live with the vault, the code source is read-only by construction, so the GitHub cache arrives later as just another read-only `repositoryRoot`. Nothing in Phases E–H depends on a remote source.
...
- **Gate closed 2026-08-11 (BP): all three criteria met.** Evidence:
  - **Configuration C precision/recall reported** via the new `evaluate proposals` (REQ-CLI-009 AC4) on the frozen corpus: overall P .538 / R .875 / F1 .667 against independent labels (P .731 / R .905 / F1 .809 with pass-two labels — 5 of the 12 independent-pass false positives are pass-two-confirmed links); per stratum, high-overlap .667/1.000, partial-overlap .429/1.000, domain-vocabulary .667/.667. Run: `gpt-5.6-luna` (BP's choice), prompt `rank-v1`, `hybrid(rrf-v1;k=60)[bm25f-v5+embed-v1:text-embedding-3-small]` over the frozen index with the Phase C embedding cache (zero embedding calls, zero corpus transmission). This discharges the Phase A deferrals: PQ3 via the proposal metrics, PQ4 via the usage ledger — 12 calls, 120 proposals, 0 malformed responses, measured $0.0213 at BP's rates (32,809 in / 12,247 out; input within 6% of the projection, output ~70% over it — the 60-token-per-candidate allowance is too small for this model and should be revisited).
  - **Full loop works, override-rate data recorded:** BP reviewed all 57 queued configuration-C proposals in `spectrace review`: 53 accept / 4 reject / 0 redirect. Every rejection sits in the review band (4 of its 6 entries; 0 of 51 suggest-band entries rejected) — the bands' queue-level triage separated the doubtful proposals. Threshold caveat handed to the chat review (REQ-CORE-041): every `implements`-classified proposal scored ≥ .75, so the suggest-only breakdown is identical to overall; at these thresholds the bands separate `supports` from `implements` but not good `implements` from bad.
  - **Studio ran the same analysis:** shipped default (configuration A) over the same corpus, requirements as the vault and the pinned hookable checkout linked via REQ-APP-015. Symbol index byte-identical (sha256-equal to `spectrace index` over the same checkout; structurally identical, 49/49, to the frozen legacy-format Phase A index). Retrieval identical in candidate identity and rank, 120/120; 14 scores differ by ≤ 2.5×10⁻¹⁶ (1 ULP, floating-point summation order between the in-memory and round-tripped-index paths — selection unaffected). Both ranked runs: same prompt version, model, commit; 120 proposals, 0 failures. Artifact envelopes are frozen cross-package contracts (`packages/cli/test/snapshots/`). BP additionally triaged Studio's whole queue (54/54) — REQ-APP-013's flagship flow exercised ahead of Phase E; recorded as a UI exercise, not as override-rate evidence.
- **Carried forward, not gate-blocking:** REQ-APP-012 stays `partial` pending BP's reading of AC1's byte-for-byte clause against the 1-ULP retrieval delta (index and envelope bytes hold exactly; ranked responses are provider-nondeterministic, so prompt-side identity is what the machinery can guarantee). Studio gains hybrid retrieval (embedding provider + a designed corpus-transmission consent moment) and run-panel pricing post-gate — both flagged 2026-08-11 when BP hit the gaps live. REQ-APP-010/011 are descoped to R1.1 per the note above.
